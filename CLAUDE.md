...
- 120 total pad slots (12 per bank × 10 banks)
- Sub Pad: hardware retrigger button (rapid-repeat of last-pressed pad for rolls/fills) — NOT a sample slot
- Example: `G0000007.WAV` = Bank G, Pad 7
- **Banks A-B are internal memory** — survive SD card swaps. Banks C-J live on the SD card.
...
- Sub Pad is a hardware retrigger button (rolls/fills), not a sample slot.

**Internal memory strategy:** Banks A-B survive SD card swaps. Put your always-need-it sounds here (core drum kit, transitions/utility). Banks C-J change with the card — use for genre-specific or session-specific content.
...
5. If zero candidates found: returns empty result (UI shows "no match" state)
...
Generated by `scripts/gen_padinfo.py` — auto-sets gate mode for pads 1-4, loop mode for pads 5-12.
...
- 12 pads per bank (Sub Pad is a retrigger button, not a sample slot — not 16 like MK2)
- Banks A-B are internal memory (survive SD card swaps), C-J are on SD card
...
### Primary Reference
**SP-404A Field Manual** (`docs/SP404A_Field_Manual.md`) — Complete hardware reference, all 29 effects with knob assignments, resample techniques, performance cheats, pad layouts, and Jambox pipeline overview. Also includes historical context on the SP-404 scene (Dibiase, Ras G, Nujabes, Knxwledge, Low End Theory). This is the #1 LLM training data source for SP-404 domain knowledge.

### LLM Training Data Sources (from ecosystem research)
These should feed the RAG corpus and/or synthetic training examples:
- **SP-404A Field Manual** — Complete effects reference, resample chains, performance techniques, cultural context
