...
from wav_utils import build_sp404_wav_from_samples

def save_wav(filename, data, pad_number=None):
    os.makedirs(OUT, exist_ok=True)
    data = np.clip(data, -1.0, 1.0)
    samples = (data * 32767).astype(np.int16)
    if pad_number is not None:
        # Build SP-404 compatible WAV with RLND chunk
        wav_data = build_sp404_wav_from_samples(samples, 'B', pad_number)
...
def main():
    ...
    save_wav("01_air_horn.wav", norm(horn), pad_number=1)
    ...
    save_wav("02_record_scratch.wav", norm(scr+crk), pad_number=2)
    ...
    las*=np.exp(-5*t)
    save_wav("03_laser_zap.wav", norm(las), pad_number=3)
    ...
    save_wav("04_robot_yeah.wav", norm(voice), pad_number=4)
    ...
    save_wav("05_siren_whoop.wav", norm(sir), pad_number=5)
    ...
    save_wav("06_8bit_powerup.wav", norm(coin), pad_number=6)
    ...
    save_wav("07_vinyl_stop.wav", norm(vs), pad_number=7)
    ...
    save_wav("08_tape_rewind.wav", norm(rew), pad_number=8)
    ...
    save_wav("09_robot_ok.wav", norm(ok), pad_number=9)
    ...
    save_wav("10_horn_stab.wav", norm(stab), pad_number=10)
    ...
    ris*=pulse
    save_wav("11_riser_buildup.wav", norm(ris), pad_number=11)
    ...
    save_wav("12_impact_drop.wav", norm(imp), pad_number=12)
