...
def copy_wav(src, dest_folder, counts, prefix=""):
...
def main():
    ...
    for wav in glob.glob(os.path.join(drum_base, "Assorted Hits/Kicks/*.wav")):
        copy_wav(wav, os.path.join(LIB, "Drums/Kicks"), counts, "MR")
    for wav in glob.glob(os.path.join(drum_base, "Assorted Hits/Snares/*.wav")):
        copy_wav(wav, os.path.join(LIB, "Drums/Snares-Claps"), counts, "MR")
    for wav in glob.glob(os.path.join(drum_base, "Assorted Hits/Hi Hats/*.wav")):
        copy_wav(wav, os.path.join(LIB, "Drums/Hi-Hats"), counts, "MR")
    for wav in glob.glob(os.path.join(drum_base, "Assorted Hits/Cymbals/*.wav")):
        copy_wav(wav, os.path.join(LIB, "Drums/Percussion"), counts, "MR-cymbal")
    ...
    for kit_dir in glob.glob(os.path.join(drum_base, "Drum Kits/*")):
        for wav in glob.glob(os.path.join(kit_dir, "*.wav")):
            ...
            if "kick" in fname or "bd" in fname:
                copy_wav(wav, os.path.join(LIB, "Drums/Kicks"), counts, os.path.basename(kit_dir))
            elif "snare" in fname or "sd" in fname or "clap" in fname:
                copy_wav(wav, os.path.join(LIB, "Drums/Snares-Claps"), counts, os.path.basename(kit_dir))
            elif "hat" in fname or "hh" in fname:
                copy_wav(wav, os.path.join(LIB, "Drums/Hi-Hats"), counts, os.path.basename(kit_dir))
            else:
                copy_wav(wav, os.path.join(LIB, "Drums/Percussion"), counts, os.path.basename(kit_dir))
    ...
    for wav in glob.glob(os.path.join(SRC, "lofi-samples/musicradar-lofi-samples/Lo-Fi DrumLoops/*.wav")):
        copy_wav(wav, os.path.join(LIB, "Drums/Drum-Loops"), counts, "lofi")

    # Hip-hop beats
    for wav in glob.glob(os.path.join(hh_base, "Beats/**/*.wav"), recursive=True):
        copy_wav(wav, os.path.join(LIB, "Drums/Drum-Loops"), counts, "HH")

    # Synth percussion loops
    for wav in glob.glob(os.path.join(SRC, "synth-percussion/**/*.wav"), recursive=True):
        copy_wav(wav, os.path.join(LIB, "Drums/Drum-Loops"), counts, "synthperc")
    ...
    for bpm_dir in glob.glob(os.path.join(funk_base, "*bpm*")):
        ...
        for wav in glob.glob(os.path.join(bpm_dir, "Bass/*.wav")):
            copy_wav(wav, os.path.join(LIB, "Melodic/Bass"), counts, f"funk-{bpm}")
        for wav in glob.glob(os.path.join(bpm_dir, "Guitar/*.wav")):
            copy_wav(wav, os.path.join(LIB, "Melodic/Guitar"), counts, f"funk-{bpm}")
        for wav in glob.glob(os.path.join(bpm_dir, "Piano/*.wav")):
            copy_wav(wav, os.path.join(LIB, "Melodic/Keys-Piano"), counts, f"funk-{bpm}")
        for wav in glob.glob(os.path.join(bpm_dir, "Electric Piano/*.wav")):
            copy_wav(wav, os.path.join(LIB, "Melodic/Keys-Piano"), counts, f"funk-{bpm}-ep")
        for wav in glob.glob(os.path.join(bpm_dir, "Organ/*.wav")):
            copy_wav(wav, os.path.join(LIB, "Melodic/Keys-Piano"), counts, f"funk-{bpm}-organ")
        for wav in glob.glob(os.path.join(bpm_dir, "Clavinet/*.wav")):
            copy_wav(wav, os.path.join(LIB, "Melodic/Keys-Piano"), counts, f"funk-{bpm}-clav")
        for wav in glob.glob(os.path.join(bpm_dir, "Monosynth/*.wav")):
            copy_wav(wav, os.path.join(LIB, "Melodic/Synths-Pads"), counts, f"funk-{bpm}")
    ...
    for bpm_dir in glob.glob(os.path.join(sf_base, "soul-n-funk-*")):
        ...
        for sub in glob.glob(os.path.join(bpm_dir, "*")):
            ...
            for wav in glob.glob(os.path.join(sub, "*.wav")):
                if "bass" in subname:
                    copy_wav(wav, os.path.join(LIB, "Melodic/Bass"), counts, f"soul-{bpm}")
                elif "guitar" in subname:
                    copy_wav(wav, os.path.join(LIB, "Melodic/Guitar"), counts, f"soul-{bpm}")
                elif "organ" in subname or "rhodes" in subname or "piano" in subname:
                    copy_wav(wav, os.path.join(LIB, "Melodic/Keys-Piano"), counts, f"soul-{bpm}")
                elif "beat" in subname or "drum" in subname:
                    copy_wav(wav, os.path.join(LIB, "Drums/Drum-Loops"), counts, f"soul-{bpm}")
    ...
    for wav in glob.glob(os.path.join(synth_base, "Polys and Pads/**/*.wav"), recursive=True):
        copy_wav(wav, os.path.join(LIB, "Melodic/Synths-Pads"), counts, "80s")
    for wav in glob.glob(os.path.join(synth_base, "Arps and Leads/**/*.wav"), recursive=True):
        copy_wav(wav, os.path.join(LIB, "Melodic/Synths-Pads"), counts, "80s-lead")
    for wav in glob.glob(os.path.join(synth_base, "Bass Loops/**/*.wav"), recursive=True):
        copy_wav(wav, os.path.join(LIB, "Melodic/Bass"), counts, "80s")

    # Hip-hop loops and single shots
    for wav in glob.glob(os.path.join(hh_base, "Loops/**/*.wav"), recursive=True):
        fname = os.path.basename(wav).lower()
        if "bass" in fname:
            copy_wav(wav, os.path.join(LIB, "Melodic/Bass"), counts, "HH")
        elif "key" in fname or "piano" in fname or "rhodes" in fname:
            copy_wav(wav, os.path.join(LIB, "Melodic/Keys-Piano"), counts, "HH")
        elif "synth" in fname or "pad" in fname:
            copy_wav(wav, os.path.join(LIB, "Melodic/Synths-Pads"), counts, "HH")
        else:
            copy_wav(wav, os.path.join(LIB, "Loops/Instrument-Loops"), counts, "HH")

    for wav in glob.glob(os.path.join(hh_base, "Single Shots/**/*.wav"), recursive=True):
        copy_wav(wav, os.path.join(LIB, "SFX/Stabs-Hits"), counts, "HH")
    ...
    for wav in glob.glob(os.path.join(SRC, "lofi-samples/musicradar-lofi-samples/ConstructionKits/**/*.wav"), recursive=True):
        copy_wav(wav, os.path.join(LIB, "Loops/Instrument-Loops"), counts, "lofi")
    for wav in glob.glob(os.path.join(SRC, "lofi-samples/musicradar-lofi-samples/SoundFX/*.wav")):
        copy_wav(wav, os.path.join(LIB, "SFX/Stabs-Hits"), counts, "lofi")
    for wav in glob.glob(os.path.join(SRC, "lofi-samples/musicradar-lofi-samples/BonusMultiSamples/**/*.wav"), recursive=True):
        ...
        if "bass" in fname or "Bass" in os.path.dirname(wav):
            copy_wav(wav, os.path.join(LIB, "Melodic/Bass"), counts, "lofi-multi")
        else:
            copy_wav(wav, os.path.join(LIB, "Melodic/Synths-Pads"), counts, "lofi-multi")
    ...
    for kit_dir in glob.glob(os.path.join(idm_base, "Kit*")):
        ...
        for wav in glob.glob(os.path.join(kit_dir, "**/*.wav"), recursive=True):
            ...
            if "drum" in fname or "beat" in fname or "loop" in fname:
                copy_wav(wav, os.path.join(LIB, "Drums/Drum-Loops"), counts, f"IDM-{kit_name}")
            elif "bass" in fname:
                copy_wav(wav, os.path.join(LIB, "Melodic/Bass"), counts, f"IDM-{kit_name}")
            elif "pad" in fname or "synth" in fname or "atmos" in fname:
                copy_wav(wav, os.path.join(LIB, "Melodic/Synths-Pads"), counts, f"IDM-{kit_name}")
            else:
                copy_wav(wav, os.path.join(LIB, "Loops/Instrument-Loops"), counts, f"IDM-{kit_name}")
