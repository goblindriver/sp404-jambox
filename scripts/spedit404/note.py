# Vendored from spEdit404, patched for banks A-J
...
class Note:
    def __init__(self, pad, bank, start_tick, length, velocity):
        ...
        # Patched: accept banks a-j (10 banks) instead of a-h (8)
        if 0 <= ord(bank.lower()) - constants.ascii_character_offset < constants.number_of_banks:
            self.bank = bank.lower()
        else:
            raise ValueError('bank must be a letter between a-j')
